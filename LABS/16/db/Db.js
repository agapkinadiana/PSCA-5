...
//запросы — с их помощью получают данные с сервера.
...
class Db {
    ...
    getOne(tableName, fields) {
        return connectionPool.then(pool => {
            ...
            Object.keys(fields).forEach(field => {       //принимает объект (афакульти) и возвращает массив полей (айди, нэйм) этого объекта
                let fieldType = Number.isInteger(fields[field]) ? sql.Int : sql.NVarChar;
                request.input(field, fieldType, fields[field]); //add input parameter to the request
